...
export default function Login() {
  ...
  const handleLogin = (e) => {
    ...
    const user = mockUsers[lowerRole];

    if (user && user.email === email && user.password === password) {
      localStorage.setItem("user", JSON.stringify({ email, role: lowerRole }));
      setLoading(false);
      if (lowerRole === "admin") {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#0e0e0e] to-[#e23535f8] flex flex-col items-center justify-center px-4">
