...
export default function EmployeeRegistration() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await fetch("/api/employees/register", {
        ...
        body: JSON.stringify({ name, email, password, role, createdAt: new Date().toISOString() }),
      });
  ...
  return (
    <form
      ...
      className="bg-gradient-to-br from-[#060110] via-[#0b0515] to-[#f40606] p-8 rounded-xl max-w-md mx-auto mt-10 space-y-6 shadow-lg text-white"
    >
      ...
      {message && (
        <p className={`text-center mb-4 ${message.startsWith("✅") ? "text-green-400" : "text-red-400"}`} role="alert" aria-live="polite">
          {message}
        </p>
      )}
